...
function sendConfirmationEmail($email, $clubName, $date, $time, $priceSubTotal, $priceIVA, $priceTotal, $className) {
                              ...
                              <tr>
                                <td class="pad">
                                  <h1
                                    ...
                                    "
                                  >
                                    ¡Listo! Tu clase ha sido registrada con
                                    éxito.
                                  </h1>
                                </td>
                              </tr>
...
if ($method == 'POST') {
    ...
    if (isset($params['id_platforms_fields_classes_users']) && isset($params['active'])) {
        ...
        if ($stripe_id) {
            $sql = "UPDATE `platforms_fields_classes_users` SET `active` = ?, `stripe_id` = ? WHERE `id_platforms_fields_classes_users` = ?";
            $stmt = $conn->prepare($sql);
            $stmt->bind_param("isi", $active, $stripe_id, $id_platforms_fields_classes_users);
        } else {
            $sql = "UPDATE `platforms_fields_classes_users` SET `active` = ? WHERE `id_platforms_fields_classes_users` = ?";
            $stmt = $conn->prepare($sql);
            $stmt->bind_param("ii", $active, $id_platforms_fields_classes_users);
        }
        ...
        if ($stmt->execute()) {
            ...
            $stmt->close();
              ...
              if ($active == 1) {
                  ...
                  $sql = "SELECT p.title, a.email, b.id_platforms_field, p.id_platform, s.platforms_date_time_start, s.platforms_date_time_end, f.title as 'cancha', b.title as 'event_title', s.price
                          FROM platforms_fields_classes_users s 
                          JOIN platforms_users a ON a.id_platforms_user=s.id_platforms_user 
                          JOIN platforms_disabled_dates b ON b.id_platforms_disabled_date=s.id_platforms_disabled_date
                          JOIN platforms_fields f ON f.id_platforms_field = b.id_platforms_field
                          JOIN platforms p ON f.id_platform = p.id_platform 
                          WHERE s.id_platforms_fields_classes_users = ?";
                  $stmt = $conn->prepare($sql);
                  ...
                  if ($data) {
                      ...
                      $className = $data['event_title'];

                      sendConfirmationEmail($email, $clubName, $date, $time, $priceSubTotal, $priceIVA, $priceTotal, $className);
