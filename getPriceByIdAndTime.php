...
if ($method == 'POST') {
    ...
    if ((isset($params['id_platforms_field']) || isset($params['id_platforms'])) && isset($params['time'])) {
        ...
        if ($id_platforms_field) {
            // Query to fetch special price (active = 2) that fits the time range by id_platforms_field
            $sql = "SELECT a.id_platforms_fields_price, a.id_platforms, a.price, a.active
                    FROM platforms_fields_prices as a
                    WHERE a.id_platforms_field = ? AND a.active = 2 AND TIME(a.platforms_fields_price_start_time) <= ? AND TIME(a.platforms_fields_price_end_time) >= ?";
            $stmt = $conn->prepare($sql);
            $stmt->bind_param("iss", $id_platforms_field, $time, $time);
        } else {
            // Query to fetch special price (active = 2) that fits the time range by id_platforms
            $sql = "SELECT a.id_platforms_fields_price, a.id_platforms, a.price, a.active
                    FROM platforms_fields_prices as a
                    WHERE a.id_platforms = ? AND a.active = 2 AND TIME(a.platforms_fields_price_start_time) <= ? AND TIME(a.platforms_fields_price_end_time) >= ?";
            $stmt = $conn->prepare($sql);
